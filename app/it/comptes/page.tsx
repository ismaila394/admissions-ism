...
export default function ITComptes() {
  ...
  const loadApplications = () => {
    const apps = dataStore.getApplications({ status: "en_attente_it" })
  ...
  const openModal = (app: Application) => {
    ...
    const password = `ISM2026${app.nom.substring(0, 2).toUpperCase()}`
    const cardNumber = `ETU-2026-${app.numero.split("-").pop()}`
    
    setFormData({
      emailISM: `${emailBase}@ism.sn`,
      motDePasseTemp: password,
      numeroCarteEtudiant: cardNumber,
    })
    setShowModal(true)
  }
